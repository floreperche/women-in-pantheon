<script>
  export let person;
  let purplePetals = [];
  purplePetals.length = 4;
  let orangePetals = [];
  orangePetals.length = 9;
</script>

<defs>
  <linearGradient id="orangeGrad" x1="0%" x2="0%" y1="100%" y2="0%">
    <stop offset="0%" stop-color="#F34C63" />
    <stop offset="40%" stop-color="#120833" />
  </linearGradient>
  <linearGradient id="purpleGrad" x1="0%" x2="0%" y1="100%" y2="0%">
    <stop offset="0%" stop-color="#7480D2" />
    <stop offset="70%" stop-color="#120833" />
  </linearGradient>
</defs>
{#if person.sex === "W"}
  {#each orangePetals as petal, i}
    <path
      fill={person.status === "Panthéonisée" ? "#F34C63" : "#120833"}
      stroke={person.status === "Panthéonisée" ? "none" : "url(#orangeGrad)"}
      stroke-width="1"
      d="M20.7,2.5C20.7,15.4,10.4,30.8,2.4,30.8C-5.7,30.8,-11.3,15.4,-11.3,2.5C-11.3,-10.3,-5.7,-20.7,2.4,-20.7C10.4,-20.7,20.7,-10.3,20.7,2.5Z"
      transform="rotate({80 * i})"
      fill-opacity={person.status === "Panthéonisée" ? "50%" : "100%"}
    />
  {/each}
{:else if person.sex === "M"}
  {#each purplePetals as petal, i}
    <path
      fill={person.status === "Panthéonisée" ? "#7480D2" : "#120833"}
      stroke={person.status === "Panthéonisée" ? "none" : "url(#purpleGrad)"}
      stroke-width="3"
      d="M88 14C88 50 44 100 15.7 100C-12.5 100 -25 50 -25 14C-25 -22 -12.5 -44 15.7 -44C44 -44 88 -22 88 14"
      transform="rotate({90 * i})scale(0.3)"
      fill-opacity={person.status === "Panthéonisée" ? "50%" : "10%"}
    />
  {/each}
{/if}

<path
  fill={person.status === "Panthéonisée"
    ? person.sex === "M"
      ? "#8891d1"
      : "#f55f74"
    : "none"}
  stroke={person.status != "Panthéonisée"
    ? person.sex === "M"
      ? "#8891d1"
      : "#f55f74"
    : "none"}
  stroke-width="6"
  d="M48.1,-66.9C63,-55.4,76.2,-42.2,82.6,-26C89,-9.8,88.5,9.4,81.8,25.5C75,41.5,62,54.4,47.3,62.6C32.6,70.8,16.3,74.3,0.2,74C-15.9,73.7,-31.7,69.6,-43.7,60.6C-55.7,51.5,-63.9,37.4,-71,21.6C-78.2,5.8,-84.4,-11.8,-81.9,-28.7C-79.3,-45.6,-68,-61.8,-52.9,-73.2C-37.8,-84.6,-18.9,-91.3,-1.1,-89.7C16.6,-88.2,33.2,-78.4,48.1,-66.9Z"
  transform="scale(0.1)"
  fill-opacity={person.status === "Panthéonisée" ? "100%" : "80%"}
/>
